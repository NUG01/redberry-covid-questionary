    ...
    <Form @submit="onSubmit" class="flex flex-col w-[100%] mt-[4.2rem]">
     ...
     <div class="flex flex-col gap-[8px]">
      ...
      <div class="flex flex-col gap-[1.1rem]">
        <radio-cmp @radio-data="yesValue" rules="required" value="yes" type="radio" name="vaccine" id="vaccine1" label="კი"/>
        <radio-cmp @radio-data="noValue" rules="required" value="no" type="radio" name="vaccine" id="vaccine2" label="არა"/>
       <ErrorMessage style="color:#F15524;font-size:16px;margin-left:2rem" name="vaccine"/>
      </div>
      </div>
      <div v-if="showFirstInputs" class="flex flex-col gap-[8px]">
      <p class="mb-[1.3rem]">აირჩიე რა ეტაპზე ხარ*</p>
      <div class="flex flex-col gap-[1.1rem]">
        <radio-cmp @radio-data="registered" rules="required" type="radio" name="stage" value="first_dose_and_registered" id="stage1" label="პირველი დოზა და დარეგისტრირებული ვარ მეორეზე"/>
        <radio-cmp @radio-data="vaccinated" rules="required" type="radio" name="stage" value="fully_vaccinated" id="stage2" label="სრულიად აცრილი ვარ"/>
        <radio-cmp @radio-data="dose" rules="required" type="radio" name="stage" value="first_dose_and_not_registered" id="stage3" label="პირველი დოზა და არ დავრეგისტრირებულვარ მეორეზე"/>
       <ErrorMessage style="color:#F15524;font-size:16px;margin-left:2rem" name="stage"/>
      </div>
      </div>
      <div v-if="showSecondInputs" class="flex flex-col gap-[8px]">
      <p class="mb-[1.3rem]">რას ელოდები?*</p>
      <div class="flex flex-col gap-[1.1rem]">
        <radio-cmp @radio-data="waiting" rules="required" type="radio" value="registered_and_waiting" name="wait" id="wait1" label="დარეგისტრირებული ვარ და ველოდები რიცხვს"/>
        <radio-cmp @radio-data="notPlanning" rules="required" type="radio" value="not_planning" name="wait" id="wait2" label="არ ვგეგმავ"/>
        <radio-cmp @radio-data="planning" rules="required" type="radio" value="planning_vaccination" name="wait" id="wait3" label="გადატანილი მაქვს და ვგეგმავ აცრას"/>
       <ErrorMessage style="color:#F15524;font-size:16px;margin-left:2rem" name="wait"/>
      </div>
      </div>
